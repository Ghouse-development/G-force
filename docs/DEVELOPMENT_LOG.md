...
## 2026年1月4日 - イベント管理機能追加

### 概要

見学会・セミナー等のイベント予約管理機能を実装。カレンダービューでの一覧表示、予約管理、参加状況・会員化追跡が可能に。

### 新規作成ファイル

| ファイル | 機能 |
|---------|------|
| `types/events.ts` | イベント・予約の型定義 |
| `store/event-store.ts` | Zustandストア（モックデータ生成含む） |
| `app/events/page.tsx` | カレンダービュー付き一覧 |
| `app/events/[id]/page.tsx` | イベント詳細・予約管理 |
| `app/events/day/[date]/page.tsx` | 日別イベント一覧 |
| `app/events/new/page.tsx` | イベント新規作成 |
| `app/events/[id]/edit/page.tsx` | イベント編集・削除 |

### 機能詳細

**カレンダービュー:**
- 月間カレンダーでイベント表示
- モデルハウス・イベント種別でフィルタリング
- 月間統計（イベント数、予約数、参加者、会員化、会員化率）

**予約管理:**
- 予約ステータス: 予約済 → 確認済 → 参加/不参加/キャンセル
- 会員化ステータス: 未確認 → 会員化/見送り
- 講師割り当て

**イベント種別:**
- MH見学会、構造見学会、完成見学会、セミナー、相談会、その他
...
「情報収集」メニュー内に「イベント管理」を追加（CalendarDaysアイコン）

---

## 2026年1月4日 - 書類作成ページ最適化・顧客詳細レイアウト改善

### 概要

書類作成ページの簡素化、顧客詳細ページのレイアウトバランス改善を実施。

### 変更内容

**書類作成ページ:**
- プラン依頼・契約の「新規作成」ボタンを顧客一覧へリンク（邸名から作成を推奨）
- カンバンビューを削除（リスト表示のみ）
- 不要な統計（競合勝ち/負け等）を削除

**顧客詳細ページ:**
- グリッド比率を 33/66 → 40/60 に変更（`lg:grid-cols-5` with col-span-2/3）
- 右側セクション（受付/ヒアリング/書類/土地/メモ/AI）を3×2グリッドカード形式に変更

### 変更ファイル

- `app/plan-requests/page.tsx` - カンバン削除、リンク変更
- `app/contracts/page.tsx` - カンバン削除
- `app/customers/[id]/page.tsx` - グリッド比率変更、カード形式化
